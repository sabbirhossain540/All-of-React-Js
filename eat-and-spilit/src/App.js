  ...
  return(
    <div className="app">
      <div className="sidebar">
        ...
        {showAddFriend && <FormAddFried onAddFriend={handleFriends}/>}
        <Button onClick={handleShowAddFriend}>{showAddFriend ? "Close" : "Add Friend"}</Button>
      </div>

      {selectedFriend && <FormSplitBill selectedFriend={selectedFriend} onSpilitBill={handleSpilitBill}/>}
    </div>
  );
}
...
function Freind({friend, onSelection, selectedFriend}){
  ...
  return <li className={isSelected? "selected":""}>
    ...
    {friend.balance < 0 && (
        <p className="red">
          You owe {friend.name} {friend.balance}  yen
        </p>
      )}

    {friend.balance > 0 && (
        <p className="green">
          {friend.name} owe you {friend.balance} yen
        </p>
      )}

    {friend.balance === 0 && (
        <p >
          You and {friend.name} even
        </p>
      )}
